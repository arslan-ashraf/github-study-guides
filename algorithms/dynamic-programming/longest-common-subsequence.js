...
function backtrack(str_a, str_b, cache, i, j){
	...
	if (str_a[i] == str_b[j]){	
		...
		let longest_matching_subsequence = Math.max(left_remaining_match, right_remaining_match)
		
		cache[cache_key] = longest_matching_subsequence
		return longest_matching_subsequence
