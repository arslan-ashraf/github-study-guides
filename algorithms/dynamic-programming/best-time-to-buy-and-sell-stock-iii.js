function best_time_to_buy_and_sell_stock_iii(prices, num_transactions = 2){
	...
	for (let i = 1; i < prices.length; i++){
		for (let k = 1; k <= num_transactions; k++){
			minimum_price[k] = Math.min(minimum_price[k], prices[i] - dp[k - 1][i - 1])
			dp[k][i] = Math.max(dp[k][i - 1], prices[i] - minimum_price[k])
		}
	}
...
console.log(best_time_to_buy_and_sell_stock_iii([ 3, 3, 5, 0, 0, 3, 1, 4 ]) == 6)
...
console.log(best_time_to_buy_and_sell_stock_iii_space_optimized([ 3, 3, 5, 0, 0, 3, 1, 4 ]) == 6)